...
function OtpInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const handleKey = (e: React.KeyboardEvent<HTMLInputElement>, idx: number) => {
    ...
    if (key === "Backspace") {
      ...
      const prev = document.getElementById(`arp-otp-${idx - 1}`) as HTMLInputElement | null;
      ...
      if (idx < 5) {
        const next = document.getElementById(`arp-otp-${idx + 1}`) as HTMLInputElement | null;
        if (next) next.focus();
      }
    }
  };
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    onChange(pasted);
    const el = document.getElementById(`arp-otp-${Math.min(pasted.length, 5)}`) as HTMLInputElement | null;
    if (el) el.focus();
  };
      ...
      {digits.map((d, i) => (
        <input
          ...
          className={`w-11 h-12 text-center text-lg font-bold border-2 rounded-xl transition-colors outline-none
            ${d.trim() ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-300 bg-white"}
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-5/12 xl:w-1/2 flex-col justify-center bg-gradient-to-br from-blue-700 via-blue-600 to-indigo-700 p-10 text-white">
        ...
        <p className="text-blue-200 text-base leading-relaxed max-w-sm">
          Enter the code from your email and choose a strong new password for your admin account.
        </p>
          ...
          {["Code", "Password", "Done"].map((label, i) => (
            <div key={i} className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-full text-xs font-bold flex items-center justify-center transition-colors
                ${i < step ? "bg-white text-blue-700" : i === step ? "bg-white/30 text-white border-2 border-white" : "bg-white/10 text-white/50"}`}>
                {i < step ? "✓" : i + 1}
              </div>
        ...
        <div className="w-full max-w-md">
              ...
              {!emailParam && (
                <div>
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                    placeholder="admin@yourpg.com" className={INPUT} />
                </div>
              )}
          ...
          {step === 1 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-sm text-gray-500">Choose a strong password for your admin account.</p>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showPw ? "text" : "password"} value={password}
                    ...
                    placeholder="Min. 6 characters" className={`${INPUT} pr-14`} />
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showConf ? "text" : "password"} value={confirm}
                    ...
                    placeholder="Re-enter your password" className={`${INPUT} pr-14`} />
          ...
          {step === 2 && (
            <div className="text-center space-y-5">
              ...
              <p className="text-gray-500 text-sm">Your admin password has been updated. You can now sign in with your new password.</p>
