...
export default function EditPG() {
  ...
  const fetchPG = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Set form values
        setValue("name", pgData.name);
        setValue("onlinePayment", pgData.onlinePayment);
        setValue("isPrivate", pgData.isPrivate || false);

        // Set location
        setLocation({
          subcity: pgData.location?.subcity || "",
  ...
  const onSubmit = async (data: PGForm) => {
    ...
    try {
      ...
      // Call the PG update API
      console.log("Updating PG with data:", updatedPG);

      const response = await pgAPI.updatePG(pgId, updatedPG);
      ...
      if (response.success) {
        ...
        setError(response.message || "Failed to update PG. Please try again.");
      }
    } catch (err: any) {
      ...
      if (err.message && err.message.includes("401")) {
        ...
        setError(err.message || "Failed to update PG. Please try again.");
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">{t("editPG.title")}</h1>
        <div className="flex gap-4">
          <Button
            variant="outline"
            onClick={() => router.push("/admin/dashboard")}
          >
            ← {t("editPG.backToDashboard")}
          </Button>
          <Button
            variant="outline"
            onClick={() => router.push(`/admin/pgs/${pgId}`)}
          >
            {t("editPG.viewPG")}
          </Button>
        </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex gap-4">
          <Button type="submit" disabled={loading}>
            {loading ? t("editPG.updatingPG") : t("editPG.updatePG")}
          </Button>
          <Button
            type="button"
            variant="outline"
