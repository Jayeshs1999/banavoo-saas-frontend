  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400 mt-0.5">Your conversations with PG owners</p>
      </div>
      ...
      {!loading && !error && convs.length === 0 && (
        <div className="flex flex-col items-center gap-3 py-20 text-center">
          ...
          <p className="text-sm text-gray-400">No conversations yet.<br />After booking a PG, you can message the owner here.</p>
        </div>
      )}
      ...
      <div className="space-y-2">
        {convs.map((c) => (
          <Link
            key={c.bookingId}
            href={`/user/chat?bookingId=${c.bookingId}`}
            className="flex items-center gap-4 p-4 bg-white border border-gray-200 rounded-xl hover:border-primary/30 hover:bg-primary/5 transition-colors"
          >
            {/* PG photo or letter avatar */}
            <div className="w-11 h-11 rounded-xl overflow-hidden bg-primary/10 flex items-center justify-center shrink-0">
              {c.pgPhoto ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={c.pgPhoto} alt={c.pgName} className="w-full h-full object-cover" />
              ) : (
                <span className="text-primary font-bold text-base">
                  {(c.pgName?.[0] ?? "P").toUpperCase()}
                </span>
              )}
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              {c.lastMessage ? (
                ...
                <p className="text-xs text-gray-400 mt-0.5">No messages yet — say hello!</p>
              )}
            </div>
